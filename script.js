const loadAllPets = async (category, sort = false) => {
      ...
      pets.forEach((pet) => {
        ...
        div.innerHTML = `
          <div class="border p-5 rounded-xl">
              ...
              <h5 class="text-xl mt-5 font-bold ">${pet_name}</h5>
              ...
              <div class="flex gap-2 justify-between gap-2">
                <div class="">
                  <i onclick="showImage('${image}')"
                    class="fa-regular fa-thumbs-up py-2 px-6 border  rounded-lg text-xl hover:bg-[#D3D3D3]"
                  ></i>
                </div>
                <button id='adopt-${petId}' onclick="showCountDown('${petId}')" class="border px-5 rounded-lg py-2  text-[#0E7A81] font-bold  hover:bg-[#D3D3D3]">
                  Adopt
                </button>
  ...
  const showImage = (img) => {
    ...
    div.innerHTML = `
      <div class="border p-1 rounded-xl">
              <img class="object-cover h-full rounded-xl w-full " src="${img}" alt="" />
            </div>
  `;
  ...
  const showDetails = (pet_information) => {
    ...
    detailsContainer.innerHTML = `
        ...
        <div>
          <h3 class="text-2xl mb-3 font-bold ">${pet_name}</h3>
